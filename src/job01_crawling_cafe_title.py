...
#크롤링할 지역과 url
local_names = ['성수', '자양', '잠실', '청담']
local_url = ['https://map.naver.com/p/search/%EC%84%B1%EC%88%98%20%EC%B9%B4%ED%8E%98?c=14.00,0,0,0,dh',
                  'https://map.naver.com/p/search/%EC%9E%90%EC%96%91%20%EC%B9%B4%ED%8E%98?searchType=place&c=15.00,0,0,0,dh',
...
try:
    for l in range(4):
        ...
        page_xpath = '//*[@id="app-root"]/div/div[4]/div[2]/a[{}]'
        cafe_titles = []
        reviews = []
        previous_num = 0
        # 1, 2페이지 누르기
        for i in range(2, 4):
            ...
            for j in range(50):
                ...
                #리뷰긁을 수 있는 프레임으로 변경
                #section_frame = driver.find_element(By.ID, 'sub_panel')
                ...
                if (j+1) % 10 == 0:
                    ...
                    df.to_csv('./crawling_data/{}_reviews{}.csv'.format(local_name, current_num), index=False)
                    previous_num = j+1
                    reviews = []
